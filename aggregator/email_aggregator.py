...
class EmailAggregator:
    ...
    #     with ProcessPoolExecutor() as executor:
    #         futures = [executor.submit(self._email_processor.process_email_file, file_path) for file_path in email_files]
    ...
    #         emails = []
    #         with ProcessPoolExecutor() as executor:
    #             futures = [executor.submit(self._email_processor.process_mbox_file, message) for message in mbox]
    ...
    def process_email_files(self, email_files):
        ...
        email_files (list): Liste des chemins des fichiers d'emails.
        """
        for file_path in tqdm(email_files, desc="Processing email files", file=sys.stdout, leave=True):
            email = self.process_email_file(file_path)
            self.add_email(email)
